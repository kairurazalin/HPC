...
void thread_func(Scene &scene, ViewPlane &viewPlane, Image &image, int sizeX, int sizeY, int numOfSamples, double s, double l){ 
...
int main(int argc, char **argv) { 
    int numOfThreads = (argc > 1 ? std::stoi(argv[1]) : 1); 
    ...
    double blockSizeX = viewPlaneResolutionX / numOfThreads; 
 
    for (int i = 0; i < numOfThreads; i++){ 
       double s = blockSizeX*i, l = blockSizeX*(i+1); 
       thread thr(thread_func, ref(scene), ref(viewPlane), ref(image), 
                 viewPlaneResolutionX, viewPlaneResolutionY, numOfSamples, s, l); 
